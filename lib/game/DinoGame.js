...
export default class DinoGame extends GameRunner {
  constructor(width, height) {
    ...
    const savedGameKeySetString = localStorage.getItem('gameKeySet');
    const savedKeySet = savedGameKeySetString !== null ? parseInt(savedGameKeySetString) : 0;
    console.log("savedKeySet", savedKeySet);
  ...
  onInput(type, key) {
    ...
    switch (type) {
      case 'jump': {
        if (state.isRunning) {
          if (this.nearestCactusKeyIs(key)) {
            if (state.dino.jump()) {
              playSound('jump')
            }
          }
  ...
  nearestCactusKeyIs(key) {
    if (this.state.cacti.length == 0) {
      return true;
    }
    return this.state.cacti[0].letter == key
  }
  ...
  increaseDifficulty() {
    ...
    if (settings.keysSet < LETTERS.length) {
      settings.keysSet++;
      localStorage.setItem('gameKeySet', settings.keysSet.toString());
      //localStorage.s
    }
  ...
  drawCacti() {
    ...
    if (this.frameCount % settings.cactiSpawnRate === 0) {
      ...
      if (!state.birds.length && randBoolean()) {
        const newCacti = new Cactus(this.spriteImageData, randItem(LETTERS[settings.keysSet]))
  ...
  drawBirds() {
    ...
    if (this.frameCount % settings.birdSpawnRate === 0) {
      ...
      if (false && randBoolean()) { // beards are not needed
  ...
  /**
   * @param {Cactus[]} instances
   */
  paintKeys(cacties) {
    for (const instance of cacties) {
      //this.paintSprite(instance.sprite, instance.x, instance.y)
      this.paintText(instance.letter.toUpperCase(), instance.x, instance.y, {
        font: 'PressStart2P',
        size: '30px',
        baseline: 'top',
        align: 'left',
